    ...
    public partial class ShaderTestBitmapsStarDiscs : Form
    {
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            using (StringFormat fmt = new StringFormat(StringFormatFlags.NoWrap) { Alignment = StringAlignment.Center, LineAlignment = StringAlignment.Center })
            {
                ...
                tim = new GLBitmapsWithStarObjects("stardiscs", rObjects, bitmapsize, new Vector3(0,2,0), 2.0f, 3, OpenTK.Graphics.OpenGL4.SizedInternalFormat.Rgba8,false, true, 2);      // group 2
                tim.CurrentGeneration = uint.MaxValue-10;
                items.Add(tim);
                tim.Add("T1", "MFred", f, Color.White, Color.Red, new Vector3(-10, 0, genpos), bannersize, new Vector3(-90F.Radians(), 0, 0), fmt, alphafadescalar: 10, alphafadepos: 5);
        ...
        private void OtherKeys( GLOFC.Controller.KeyboardMonitor kb )
        {
            if ( kb.HasBeenPressed(Keys.F1))
            {
                ...
                using (StringFormat fmt = new StringFormat(StringFormatFlags.NoWrap) { Alignment = StringAlignment.Center, LineAlignment = StringAlignment.Center })
                {
                    ...
                    Vector3 bannersize = new Vector3(width, 0, 0);
                    Font f = new Font("MS sans serif", 8f);
                    tim.Add("T1-" + genpos, "F" + genpos, f, Color.White, Color.Red, new Vector3(-10, 0, genpos), bannersize, new Vector3(-90F.Radians(), 0, 0), fmt, alphafadescalar: 10, alphafadepos: 5);
